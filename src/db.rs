...
pub async fn get_cggraph(con: web::Data<std::sync::Arc<arangors::Connection>>) -> impl Responder {
    ...
    let goals: Vec<shared::learning_trajectory::ConsensusGoal> = db
        .aql_str("for u in consensus_goals return u")
        .await
        .unwrap();
    let edges: Vec<shared::learning_trajectory::ConsensusEdge> =
        db.aql_str("for u in progs return u").await.unwrap();

    HttpResponse::Ok()
        .json::<shared::learning_trajectory::CGGraph>((goals, edges))
        .await
}
